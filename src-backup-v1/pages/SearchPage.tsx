...
export default function SearchPage() {
  ...
  const weakPointChapters = useMemo(() => {
    return data.subjects
      .flatMap(sub =>
        sub.chapters
          .filter(ch => ch.weakPoints.length > 0)
          .map(ch => ({
            subjectId: sub.id,
  ...
  return (
    <AppShell>
      ...
      <Input
        size="large"
        placeholder="搜索考点... 如「系统论」「建构主义」「AECT」"
        value={query}
      ...
      {query.trim() && (
        <div style={{ margin: '16px 0 24px' }}>
          ...
          {results.length === 0 ? (
            <Card>
              <div style={{ textAlign: 'center', padding: 24, color: 'var(--animal-text-color-secondary, #9f927d)', fontSize: 13 }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{
                    ...
                    {item.type === 'knowledge-point' ? '考点' :
                     item.type === 'weak-point' ? '薄弱' :
                     item.type === 'flashcard' ? '闪卡' : '章节'}
                  </span>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        {weakPointChapters.length === 0 ? (
          <Card>
            <div style={{ textAlign: 'center', padding: 24, fontSize: 13, color: 'var(--animal-text-color-secondary, #9f927d)' }}>
              🎉 暂无薄弱点！
            </div>
          </Card>
        ) : (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 13 }}>
                    {ch.subjectIcon} {ch.subjectName} · 第{ch.chapterOrder}章 {ch.chapterTitle}
                  </div>
                  <div style={{ fontSize: 11, color: '#fc736d', marginTop: 2 }}>
                    {ch.count} 个薄弱点待攻克
                  </div>
                </div>
